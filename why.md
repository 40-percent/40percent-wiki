...
A simple 40% keymap has two extra layers and two layer-shift keys. Holding one, often with the thumb, provides navigation, editing, numbers and functions; holding the other provides symbols. Many variations are possible, including richer navigation on a dedicated nav/edit layer or a leader key that outputs text snippets from short prefixes.

Preferred board sizes vary. An easy start is 12 to 13 keys wide. That provides the letter area plus familiar side keys for delete, shift, return, tab and esc. A bottom row holds thumb keys for layers and modifiers and may have side utility keys for arrows or extras. Using fewer keys, such as a width of 10, continues the tradeoff between smaller size and less pinkie or thumb reaching vs. increased layer complexity. Preferences also vary on how the physical keys are arranged: between row stagger, ortho-linear, or ergo/column stager and between unibody or split.
